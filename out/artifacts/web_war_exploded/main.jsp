                ...
                <div class="row">
                    <c:if test="${not empty sessionScope.last_page}">
                        ...
                        <c:if test="${sessionScope.last_page eq 'recycle'}">
                            ...
                            <div class="col-lg-2">
                                <input type="submit" class="btn btn-danger btn-sm btn-block" name="empty"
                                       value="Empty Recycle"/>
                            </div>
                        </c:if>
                    </c:if>
                    ...
                    <table class="table table-hover">
                        ...
                        <c:forEach var="task" items="${requestScope.tasks}">
                            <tr>
                                ...
                                <c:if test="${not empty sessionScope.last_page}">
                                    <td class="col5">
                                        <button type="submit" class="btn btn-default btn-sm" name="edit"
                                                value="<c:out value="${task.id}"/>">Edit
                                        </button>
                                    <td class="col5">
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <a class="btn btn-default btn-sm"
                                                   href="<c:url value='/download' ><c:param name='fileName' value="${task.fileName}" /></c:url>">Download</a> <%--${task.fileName}--%>
